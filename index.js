...
const token =
  "OUuCncH4-9COxLt2O-XNljDWctyAQtmb2A24pMJbugGTmpFkoT_aTXszOg198UZjgTF6wAAseb-UQ_Z52GxbCw==";
...
function PointMotion(dat, ip) {
  return new Point("motionReading")
    .tag("sensor", ip)
    .floatField("value", dat)
    .timestamp(new Date());
}

// Returns a point converted from noise data
function PointNoise(dat, ip) {
  return new Point("noiseReading")
    .tag("sensor", ip)
    .floatField("value", dat)
    .timestamp(new Date());
}
